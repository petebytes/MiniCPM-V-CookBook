"""
python verification_pair_list_v2.py pair.lst \
...
for t1, t2 in tqdm.tqdm(zip(tsv1, tsv2), total=len(tsv1)):
    ...
    try:
        sim, model = verification(
            ...
            wav1_end_sr=args.wav1_end_sr,
            wav2_end_sr=args.wav2_end_sr,
            model=model,
            wav2_cut_wav1=args.wav2_cut_wav1,
            device=args.device,
        )
    ...
    if sim is None:
        continue
    scores_w.write(
        f"{t1_path}_{args.wav1_start_sr}_{args.wav1_end_sr}|"
        f"{t2_path}_{args.wav2_start_sr}_{args.wav2_end_sr}\t{sim.cpu().item()}\n"
    )
